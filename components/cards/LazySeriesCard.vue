<template>
  <div ref="card" :id="`series-card-${index}`" :style="{ width: width + 'px', height: height + 'px' }" class="rounded-xl cursor-pointer z-30" @click="clickCard">
    <div class="absolute top-0 left-0 w-full box-shadow-book shadow-height" />
    <div class="w-full h-full bg-secondary relative rounded-xl overflow-hidden">
    ...
    <div v-if="seriesPercentInProgress > 0" class="absolute bottom-0 left-0 max-w-full z-10 rounded-b-xl rounded-full w-full box-shadow-progressbar" :class="isSeriesFinished ? 'bg-success' : 'bg-accent'" :style="{ width: seriesPercentInProgress * 100 + '%', height: '3px' }" />
