<template>
  <div class="relative rounded-xl overflow-hidden" :style="{ width: width + 'px', height: height + 'px' }">
    <div v-if="hasOwnCover" class="w-full h-full relative rounded-xl">
      <div v-if="showCoverBg" class="bg-secondary absolute top-0 left-0 w-full h-full">
    ...
    <div v-else-if="books.length" class="flex justify-center h-full relative bg-secondary bg-opacity-95 rounded-xl">
    ...
    <div v-else class="relative w-full h-full flex items-center justify-center p-2 bg-secondary rounded-xl">
      <div class="absolute top-0 left-0 w-full h-full bg-gray-400 bg-opacity-5" />
