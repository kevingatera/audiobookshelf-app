<template>
  <modals-bottom-sheet v-model="show" :title="`${chapters.length} ${$strings.LabelChapters}`">
    <div ref="container" class="w-full" @scroll="handleScroll">
      <div class="sticky top-0 z-10 bg-secondary grid grid-cols-[1fr_auto_auto] gap-2 px-4 py-2.5 border-b border-warm transition-shadow" :class="{ 'shadow-md': isScrolled }">
        <div>
      ...
      <ul class="w-full" role="listbox" aria-labelledby="listbox-label">
        <li
          ...
          :class="currentChapterId === chapter.id ? 'bg-bg/40' : ''"
          role="option"
          @click="clickedOption(chapter)"
        >
          <div class="grid grid-cols-[1fr_auto_auto] gap-2 px-4 py-3 items-start" :class="{ 'border-l-2': currentChapterId === chapter.id }" :style="currentChapterId === chapter.id ? 'border-left-color: #1ad691' : ''">
